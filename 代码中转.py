...
class CentralServer:
    ...
    def compute_reputation(self, performance, data_age):
        # 根据新的定义计算声誉
        return performance * 0.8 + data_age * 0.2
    ...
    def check_and_aggregate_models(self, use_reputation=False):
        ...
        print(" use reputation: ", use_reputation)
        ...
        while True:
            if self.local_models.qsize() >= 5:
                ...
                # Here you can specify whether to use reputation-based aggregation
                self.aggregate_models(models_to_aggregate, use_reputation=True)

                # Print all model IDs and their reputations
                print("All model IDs and their reputations: ", self.reputation)

                # Print the IDs of the models that participate in the aggregation
                participating_ids = [
                    list(model_dict.keys())[0] for model_dict in models_to_aggregate
                ]
                print(
                    "IDs of the models that participate in the aggregation: ",
                    participating_ids,
                )
