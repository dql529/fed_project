...
def load_dataset():
    ...
    # 处理数据以适应您的模型
    #这边可能出现问题
    pretrain_dataset = TensorDataset(torch.tensor(pretrain_data.values.reshape(-1, 18, 1), dtype=torch.float32),
...
def load_local_dataset():
    ...
    local_dataset = TensorDataset(torch.tensor(local_data.values.reshape(-1, 18, 1), dtype=torch.float32),
